...
function state__databinding() {
  for (i = 0; i < state__products.length; i++) {
    $tilte[i].innerHTML = state__products[i].title;
...
function dynamic__databinding() {
  for (i = 0; i < dynamic__products.length; i++) {
    $tilte[i].innerHTML = dynamic__products[i].title;
...
function filter__fiveWonDown() {
  let fiveBtn = document.getElementById('five-btn');
  let card = document.getElementById('card');

  fiveBtn.addEventListener('click', function () {
    let fiveWonDown = state__products.filter(product => {
      return product.price <= 60000
    });

    for (i = 0; i < fiveWonDown.length; i++) {
      if (fiveWonDown[i].price === state__products[i].price) {
